...
public class LabDataService {
    ...
    public ResponseEntity<ApiResponse> getOne(Long id) {
        Optional<LabDataBean> optionalLabDataBean = repository.findById(id);
        if (optionalLabDataBean.isPresent()) {
            Long id2 = optionalLabDataBean.get().getId();
            System.out.println(id);
            LabDataBean LabData = repository.getOne(id2);
    ...
    public ResponseEntity<ApiResponse> save(LabDataBean LabData){
        Optional<LabDataBean> optionalLabDataBean = repository.findById(LabData.getId());
        if(optionalLabDataBean.isPresent())
    ...
    @Transactional(rollbackFor = {SQLException.class})
    public ResponseEntity<ApiResponse> update(LabDataBean LabData){
        Optional<LabDataBean> foundLabData = repository.findById(LabData.getId());
